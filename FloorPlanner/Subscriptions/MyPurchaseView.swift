...
//
                    ...
                    // Force Weekly for product "003" if that's the intended weekly product
                    if product.id == subscription.prodFreeTrial {
                        duration = "week"
                        durationPlanName = String(localized: "3-Day Trial")
                    }
...
//                    }
    ...
    func calculatePercentageSaved(for duration: String) -> Int {
        ...
        if let weeklyPriceString = productDetails.first(where: {$0.duration == "week"})?.price,
           let weeklyNumber = formatter.number(from: weeklyPriceString) {
            ...
            if duration == "year",
               let yearlyPriceString = productDetails.first(where: {$0.duration == "year"})?.price,
    ...
    var body: some View {
        ...
        ZStack {
            ...
            VStack (spacing: 10) {
                ...
                // Close button or progress indicator
                HStack {
                    Spacer()
                ...
                VStack{
                    ...
                    VStack (alignment: .leading) {
                        PurchaseFeatureView(title: String(localized: "Unlimited Image generations"), icon: "photo.stack", color: color)
                        
                        PurchaseFeatureView(title: String(localized:"Remove watermark "), icon: "circle.slash", color: color)

                        
                      PurchaseFeatureView(title: String(localized: "Access to all design styles"), icon: "eraser.line.dashed", color: color)
                ...
                VStack (spacing: 20) {
                                        ...
                                        HStack {
                                            VStack(alignment: .leading) {
                                                Text(productDetails.durationPlanName)
                                                    .font(.headline.bold())
                                                if productDetails.durationPlanName == "Weekly" || productDetails.durationPlanName == "Monthly" {
                                                 
                                                    Text(productDetails.price+" "+productDetails.priceDurationInfo)
                                                        .foregroundStyle(.white)
                                                }
                                                else {
                                                    HStack (spacing: 0) {
                                                        if let referencePrice = calculateReferencePrice(for: productDetails.duration),
                                                           let referencePriceFormatted = toLocalCurrencyString(referencePrice),
                                                           referencePrice > 0
                                                        {
                                                            // Shows the reference price based on weekly calculation
                                                            // (weekly×52 for yearly)
                                            ...
                                            if productDetails.duration == "year" {
                                                VStack {
                                                    Text("SAVE \(calculatePercentageSaved(for: productDetails.duration))%")
                                                        .font(.caption.bold())
                                                        .foregroundStyle(.white)
                                                        .padding(8)
                                                }
                    ...
                    if subscription.productIds.contains(subscription.prodFreeTrial) {
                        
                        Toggle("Free trial enabled", isOn: $isFreeTrial)
                            .tint(.accent)
                    }
    ...
    func canShowProduct (product:PurchaseProductDetails) -> Bool {
   
        if self.isFreeTrial && product.id == subscription.prodFreeTrial {
            return false
        }
        
        
         if !self.isFreeTrial && product.id == subscription.prodNoTrial {
             return false
         }
